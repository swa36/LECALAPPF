...
Метка `Product.is_archive` — единая точка. Товар, помеченный в 1С на удаление
или пропавший из выгрузки, получает метку и нулевой остаток, дальше всё
расходится само:
...
Источников метки два:

- `set_catalog_data_stock` — по полю `deletion_mark` товара из `/products`
  (1С отдаёт помеченные на удаление вместе с живыми);
- `archive_missing_products` — по составу выгрузки, для товаров, которые из
  `/products` пропали совсем.

Чтобы источники не спорили, `get_data_1C` передаёт в `archive_missing_products`
только товары без `deletion_mark` — иначе тот снимал бы метку, которую тут же
возвращает `set_catalog_data_stock`, и каждый прогон писал бы в
`logs/archived_1c.log` ложный «ВОЗВРАТ». Подробности:
